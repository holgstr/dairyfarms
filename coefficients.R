##### -------------------------------------------------------------------------------------------
##### Coefficients for the Binomial and Beta Models after Variable Selection using all Predictors
##### -------------------------------------------------------------------------------------------
...
set.seed(123)
models <- lapply(dfs, fit_binomial)
...
beta_mod_hfoc <- gamlss(target ~ ., family = BEZI(mu.link = "logit"), data = dfs_imp$hf_oc, trace = FALSE)
...
beta_mod_hfuc <- gamlss(target ~ ., family = BEZI(mu.link = "logit"), data = dfs_imp$hf_uc, trace = FALSE)
...
beta_mod_simoc <- gamlss(target ~ ., family = BEZI(mu.link = "logit"), data = dfs_imp$sim_oc, trace = FALSE)
...
beta_mod_simuc <- gamlss(target ~ ., family = BEZI(mu.link = "logit"), data = dfs_imp$sim_uc, trace = FALSE)
